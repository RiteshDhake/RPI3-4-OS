...
{
    printf("Page allocator test start\n");

    void* pages[PAGE_TEST_COUNT];

    for (int i = 0; i < PAGE_TEST_COUNT; i++) {

        pages[i] = allocate_memory(PAGE_SIZE);

        if (!pages[i]) {
            printf("Page OOM at %d\n", i);
            break;
        }

        memzero((unsigned long)pages[i], PAGE_SIZE);
